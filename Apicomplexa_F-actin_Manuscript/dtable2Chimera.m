...
for i = 1:size(nTomo,1)
    ...
    if ~stat
        ...
        command = strcat('mv -f temp_dynamo_chimera_dir_transform.cmd',{' '},string(newDir));
        command2 = strcat('mv -f temp_chimera_table.em',{' '},string(newDir));
        status = system(command); 
        status = system(command2);

        %Need to convert the OG Chimera position matrix file to one that is
        %compatible with ChimeraX
        path = strcat(newDir,'/temp_dynamo_chimera_dir_transform.cmd');
        posFile = dread(path);
        newPositions = [];
        ...
        for k = 1:size(tCell{i},1)
            line1 = strcat(line1,{' '},homeDir,'/',newDir,'/temp_chimera_table.em');    
        end
        line2 = "volume #1.* level 6";
